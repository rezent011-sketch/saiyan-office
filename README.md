...
社員（実名のみ）: メインAI社員, 開発担当Bot, 開発担当Bot2, UTAGE・LINE担当Bot, TikTok LIVE切り抜きBot, 動画生成担当Bot, 広告運用Bot, 新規事業Bot, 新規顧客開拓Bot, Xマーケティング担当Bot, クラウド環境構築Bot, コンサル管理Bot, Claude Code開発。指示の担当は部屋ごとに固定（例: 司令塔→メインAI社員、AIバーチャルオフィス→開発担当Bot2）。
